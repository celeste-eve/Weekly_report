...
def generate_summary(input_file, output_dir):
    ...
    columns_to_keep = ['Project', 'Broad type', 'Test Type', 'Scheduled', 'InProgress', 'Completed', 'Approved', 'Total', 'NCF? Restricted numbers', 'Week Approved Increase']
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        for (project, test_type), group in grouped:
            sheet_name = f"{project[:15]}_{test_type[:10]}"  # Limit name length
